class Solution {
    ...
    void dijkstra(int n, List<int[]>[] adj, int[] dist, int src) {
        // update when push into heap
        PriorityQueue<int[]> pq = new PriorityQueue<>((a, b) -> (a[1] - b[1])); // min heap
        pq.add(new int[]{src, 0});
        while (!pq.isEmpty()) {
            int[] cur = pq.remove();
            int u = cur[0]; // current node
            int du = cur[1]; // current dist
            if (du > dist[u]) {continue;} // dont update until getting shorter distance
            
            for (int[] nb : adj[u]) {
                int v = nb[0]; // next node
                int duv = nb[1]; // next edge
                if (dist[v] > du + duv) {
                    dist[v] = du + duv;
    ...
    void dijkstra2(int n, List<int[]>[] adj, int[] dist, int src) {
        // update when push into heap
        PriorityQueue<int[]> pq = new PriorityQueue<>((a, b) -> (a[1] - b[1])); // min heap
        pq.add(new int[]{src, 0});
        while (!pq.isEmpty()) {
